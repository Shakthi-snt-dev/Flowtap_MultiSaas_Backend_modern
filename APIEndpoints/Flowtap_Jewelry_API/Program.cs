...
{
    app.UseSwagger();
    app.UseSwaggerUI(c => { c.SwaggerEndpoint("/swagger/v1/swagger.json", "Flowtap Jewelry API v1"); c.RoutePrefix = "swagger"; });
}
...
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "jewelry", timestamp = DateTime.UtcNow })).WithTags("Health");

try { Log.Information("Starting Flowtap Jewelry API"); await app.RunAsync(); }
catch (Exception ex) { Log.Fatal(ex, "Flowtap Jewelry API failed to start"); }
