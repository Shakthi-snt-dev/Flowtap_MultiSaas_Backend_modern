...
public class GetSentMessagesQueryHandler(IApplicationDbContext db)
    : IRequestHandler<GetSentMessagesQuery, Result<List<DirectMessageDto>>>
{
    public async Task<Result<List<DirectMessageDto>>> Handle(
        GetSentMessagesQuery request, CancellationToken ct)
    {
